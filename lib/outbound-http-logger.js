 ...
 * @private
 */
OutboundHttpLogger._stats = _.cloneDeep(DEFAULT_STATS);
 ...
 */
OutboundHttpLogger.go = OutboundHttpLogger.quickstartLogEverything = function() {
  OutboundHttpLogger.create();
  OutboundHttpLogger.enable();
};


OutboundHttpLogger.printStats = function(logger) {
  logger = logger || console.log;
  debugger;
  logger(JSON.stringify(OutboundHttpLogger._stats, null, 2));
};

OutboundHttpLogger.stats = function() {
  return _.cloneDeep(OutboundHttpLogger._stats);
};
...
OutboundHttpLogger.prototype.logRequestStart = function(requestContext) {
  ...
  if (this.options.requestHeaders) {
    httpLogObject.headers = clientRequest._headers
  }
...
OutboundHttpLogger.prototype.logRequestEnd = function(requestContext) {
  ...
  if (this.options.responseJsonBody === true) {
    ...
    if (length && type && !chunked) {
      if (type.indexOf('json') >= 0 && parseInt(length) <= this.options.jsonBodyMaxLength && message.text) {
        try {
          httpLogObject.content.body = JSON.parse(message.text);
        } catch (err) {
          httpLogObject.content.body = 'parse ERROR';
        }
      }
    }
  }
...
function hookRequest(originalFn) {
  ...
  return function requestHook(options) {
    ...
    if (typeof options === 'string' && Object.isFrozen(options)) {
     ...
     */
    function requestEnd() {
      let stats = OutboundHttpLogger._stats;

      stats.count++;

      let urlCount = stats.urls[requestContext.outboundUrl] || 0;
      stats.urls[requestContext.outboundUrl] = ++urlCount;
